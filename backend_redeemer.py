...
def _is_html_body(body: bytes) -> bool:
    """Retorna True se o body parece ser HTML de erro (nginx/proxy), não protobuf."""
    try:
        head = body[:120].decode("ascii", errors="ignore").lower()
...
async def test_code(token: str, code: str, index: int) -> tuple[str, bytes, int]:
    ...
    try:
        ...
        if n > config.SUCCESS_THRESHOLD:
            status = "valid"
            decoded = _decode_response(resp.content)
            html = _is_html_body(resp.content)
            if html:
                status = "error"
                log.warning(f"[{index:02d}] FALSO POSITIVO '{code}' — HTTP {resp.status_code} com HTML ({n}b) | decoded=[{decoded}]")
            else:
